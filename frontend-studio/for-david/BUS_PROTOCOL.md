...
3. Delivery is Tower's job (since 2026-07-14: David is the gate, never the wire). Since 2026-07-15 a mail-carrier daemon (launchd `com.davidleess.dg-mail-carrier`, every 60s, log `/tmp/dg-mail-carrier.log`) auto-submits routinely stranded inter-agent messages in idle unfocused panes; Tower still verifies its own deliveries, handles strands in David's focused pane, and adjudicates grant-shaped messages the carrier deliberately holds. On David's "relay NNN", Tower sends `From Studio — review ~/frontend-studio/proposals/NNN-RELAY.md and respond per the standing protocol` to all three crew panes (they may read `proposals/` — it contains only Studio's outward-facing work), verifies each delivery landed (input line cleared or spinner visible; retry Enter once — the tmux paste race strands messages), sends Studio the bare mechanical ack (`NNN relayed to crew`, nothing more), and confirms to David. Framing stays neutral — no lean signaled, or the review is biased.
