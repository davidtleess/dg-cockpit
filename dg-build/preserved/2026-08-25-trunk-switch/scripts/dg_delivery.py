...
def split_regions(
    ...
    WIRE-GEMINI-3 (closing-border fallback, BORDERED profiles only). The
    cursor rule assumes the CLI parks the terminal cursor on its composer.
    claude and codex do; the agy (gemini) chrome does NOT — measured live
    2026-08-06, stable across repeated probes: prompt at row 24, tmux
    ``cursor_y`` 21, parked at the end of the last rendered response line.
    The guard below then held EVERY send, the status footer counted as
    typed input, and the pane was refused ``input_not_empty`` on a visibly
    empty composer. That is the same measured cost as WIRE-GEMINI-2: a peer
    silently unreachable with the fault on our side.

    So when — and ONLY when — the cursor is unusable AND the profile is
    bordered, the region is taken from the composer's MATCHED BORDER PAIR:
    the last border line in the frame (closing), the nearest border above it
    (opening), and at least one prompt row strictly BETWEEN them. Inside that
    proven pair the region binds the FIRST prompt row, so continuation lines
    stay input. Anything short of all three conditions holds fail-closed. The
    registry already documents this as the intended bordered mechanism
    ("chrome below the closing border is excluded by GEOMETRY"); it was simply
    never implemented for the bottom edge.

    Round-6 B2 is NOT reopened: whenever the cursor is usable the cursor
    governs unchanged, which is the case B2 tests, and an interior rule can
    never bind on its own because a pair plus an enclosed prompt is required.

    Three defects Codex reproduced against the FIRST version of this fallback,
    recorded because each one was a real hazard I shipped into review:
    - W1: a lone closing rule with no opening border bound anyway.
    - W2: the prompt came from the UP-scan (the LAST prompt row), so a
      composer holding `> first line` then a bare `>` continuation bound the
      continuation and pushed the typed line into conversation, reporting
      EMPTY. That is a swallow, not a refusal.
    - W3: the residual I named was stated with its direction REVERSED — an
      extra lower border includes the intervening region and FALSE-REFUSES;
      it does not swallow. Refusing costs a re-send; swallowing costs a
      message. The bias is deliberate and in that order.
    """
    ...
    def _is_border(index: int) -> bool:
        body = strip_ghost(lines[index]).strip()
        return bool(body) and all(ch in _BORDER_CHARS for ch in body)

    def _is_prompt(index: int) -> bool:
        body = strip_ghost(lines[index]).strip().replace("\u00a0", " ")
        return any(body.startswith(prefix) for prefix in _PROMPT_LINE_PREFIXES)

    # WIRE-GEMINI-3 — the composer PAIR, for bordered profiles only. A pair is
    # proven only by: a closing border (the last border in the frame), an
    # opening border above it, and at least one prompt row strictly BETWEEN
    # them. Nothing less counts (W1: a lone closing rule is not a composer).
    # The two edges of one box are drawn IDENTICALLY by the CLI, so the
    # opening border must carry the SAME signature as the closing one. That
    # is what makes the pair a box rather than two unrelated rules, and it is
    # measured, not assumed: a live agy capture holds a 6-char separator
    # inside Gemini's own conversation output alongside the two 97-char
    # composer borders. Proximity cannot tell those apart; signature can.
    #
    # W1+W2 COMBINED (Codex counterprobe, reproduced at both cursor rows):
    # `border40 / "> first line" / rule32 / bare ">" / border40 / footer`.
    # Taking the NEAREST border as the opening picked the 32-wide interior
    # rule, bound the bare continuation prompt, and pushed the real typed
    # line into conversation reporting EMPTY — a swallow that survived the
    # separate W1 and W2 repairs because it composes them.
    opening = closing = None
    if profile.bordered:
        closing = next(
            (i for i in range(len(lines) - 1, -1, -1) if _is_border(i)), None
        )
        if closing is not None:
            # Among same-signature candidates take the EARLIEST, not the
            # nearest: Codex's rule that an ambiguous boundary must resolve
            # toward preserving MORE possible input, because over-inclusion
            # only ever causes a refusal while under-inclusion destroys a
            # message. W5 (Codex counterprobe) is the shape that forces it —
            # `history / b / "> first line" / b / ">" / b / footer` with ALL
            # THREE borders the same width. Nearest-wins bound the middle
            # border, took the bare continuation, and swallowed the typed
            # line. Earliest-wins spans the whole box and preserves it.
            #
            # I resisted earliest-wins one round earlier, on the grounds that
            # a live capture proves conversation DOES contain border lines (a
            # 6-char separator inside Gemini's own output). That objection was
            # wrong once signature matching is in place: it filters those out
            # by width, so the earliest SAME-SIGNATURE candidate is still the
            # composer's own opener on the real frame. Both guards are needed
            # and neither is sufficient alone.
            signature = strip_ghost(lines[closing]).strip()
            opening = next(
                (
                    j
                    for j in range(closing)
                    if _is_border(j) and strip_ghost(lines[j]).strip() == signature
                ),
                None,
            )
            if opening is not None and not any(
                _is_prompt(k) for k in range(opening + 1, closing)
            ):
                opening = closing = None

    # REGION START. With a proven pair the start is its opening border, which
    # binds the FIRST prompt row inside the box and therefore preserves every
    # continuation line (W2 — the swallow: the UP-scan finds the LAST prompt
    # row, so a bare ">" continuation under a typed line used to bind the
    # continuation and push the typed line into conversation, on BOTH the
    # cursor and the fallback paths). Without a pair, the legacy behaviour
    # stands: the prompt row, plus an immediately-preceding border as chrome.
    if opening is not None:
        region_start = opening
    else:
        # W6: the fail-closed HOLD path had the SAME continuation bug the pair
        # path did, and the pair fix never covered it. `history / border /
        # "> first line" / ">"` — a composer whose closing border is not
        # rendered, so no pair is provable — bound the bare continuation from
        # the UP-scan and pushed the typed line into conversation, empty=True
        # at EVERY cursor row.
        #
        # A hold only refuses, so its region may be as WIDE as needed and must
        # never be narrow. Two narrower rules were tried and BOTH still
        # swallowed, each caught by a counterprobe:
        #   - "last border above the prompt" as a floor repeated the proximity
        #     mistake: a 32-wide interior rule became the floor.
        #   - walking up across border/prompt runs stopped at `second line`,
        #     a PLAIN continuation inside the composer. Nothing distinguishes
        #     a composer continuation from conversation prose by inspection,
        #     so no upward walk can be trusted here.
        # The region therefore starts at the FIRST prompt row in the frame:
        # no prompt-bearing content can then be lost, and everything above it
        # stays conversation. Over-inclusion only ever refuses.
        #
        # This must NOT be reused for the cursor path: widening the start
        # there would drag an earlier conversation prompt row into the region
        # on unbordered profiles and refuse every codex send.
        region_start = next(
            (k for k in range(len(lines)) if _is_prompt(k)), prompt_index
        )

    # The row the region actually binds: the first prompt at/after the start.
    # Usability is measured against THIS, never against `region_start` — the
    # start can be an opening border, and a cursor resting on that border
    # would otherwise read as "usable" and truncate the region ABOVE the
    # prompt, losing the typed line into neither region. Found while fixing
    # W2; it is the same swallow wearing different clothes.
    bind_prompt = next(
        (k for k in range(region_start, len(lines)) if _is_prompt(k)), prompt_index
    )

    # BOTTOM EDGE. The cursor still governs whenever it is usable — that is
    # what keeps round-6 B2 closed, because an interior rule must never bound
    # a region the user is actively typing below.
    if cursor_row is not None and bind_prompt <= cursor_row < len(lines):
        return (
            "\n".join(lines[region_start : cursor_row + 1]),
            "\n".join(lines[:region_start]),
        )
    # Cursor unusable (the live agy state). A proven pair supplies the bound.
    if opening is not None and closing is not None:
        return "\n".join(lines[region_start:closing]), "\n".join(lines[:region_start])
    # No usable cursor and no proven pair: held — everything from the prompt
    # down is input; no chrome is excluded (never a swallow).
    return "\n".join(lines[region_start:]), "\n".join(lines[:region_start])
