...
this session; its outputs are cited in framing v5. Nothing here is tracked by git yet.
...
GENERATOR_VERSION = "fbg-identity-census/6"
...
CURRENT_FRAMING = "footballguys_adp_pilot_framing_claude_v7.md §5"

# --- SCRATCH-ROOT ALLOWLIST for --full (round-4 finding 2) ---------------------------------
# v4 refused only paths INSIDE the repository, so a Desktop path, a synced drive, or another
# checkout all passed while the artifact's own retention note says it must not be replicated
# offsite. Refusing one known-bad location is not enforcing a scratch-only contract. The
# destination is now POSITIVELY ALLOWLISTED: system-temp roots only, everything else refused.
...
def build(src: Path, full: bool) -> dict:
    ...
    doc = {
        ...
        "full_mode_destination_policy": (
            "positive allowlist: system-temp scratch roots only; repository and every other "
            "durable root refused; destination resolved before containment checks"
