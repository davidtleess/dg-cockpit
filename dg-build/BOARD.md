...
| DG-091 | First-user verdict: frontend "not great" — all four dimensions (looks, density, language, navigation) | **6** | **BRIEF DRAFTED 08-29 night (`DG091-DESIGN-BRIEF.md`) + DAVID'S PROSE RULING recorded verbatim ("no caveats/hard governance wording — prose and layman's language; world-class fantasy front end, not a data-science viz") as the brief's controlling principle.** VENUE RULED: Studio fresh-eyes (via Tower, wall respected); build post-freeze / season wk 1-2 | — |
...
| DG-100 | nflverse vintage record in NO backup; 30GB raw/ tree invisible to the anti-rot test | **1** | todo — DECIDED 08-29 (David's delegation): incremental raw/ channel + rationale update, NOT nightly full-upload (4x payload, ~333GB/mo bill growth); land pre-freeze if Wed/Thu | — |
