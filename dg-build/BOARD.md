...
| DG-134 | The forward capture's provenance records `training_cutoff: null` every morning — it reads a `season` column the runtime table lacks (`feature_season`); swallowed KeyError | **2** | **BUILT 09-02 21:58 on `ticket/DG-134` (`7fbc40e1`), not yet pushed or landed.** Root cause confirmed: `record["season"]` vs the live `feature_season` (col 19 of 44 — the ticket said 40), `KeyError` swallowed, so a null was hashed into `provenance_hash` on every row. Fix: the derivation now uses `inference_partition`'s `SEASON_COLUMN`/`ELIGIBLE_COLUMN` and its `season_of`/`is_training_eligible` normalisers (promoted public — `feature_assembly.SEASON_COLUMN` does not exist); an unanswerable table is REFUSED with the bare token `capture_training_cutoff_underivable` (`TrainingCutoffUnderivable`) → driver's own aborted report, nothing appended, `decision_supported false`; legacy refresh restores + aborts at both hash sites; production reaches it via DG-136 → exit 1. Measured on trunk's LIVE runtime pair into a scratch DB: HEAD `{value: null}` / hash `9b8fd7bf…` → DG-134 `{value: 2023, status: derived}` / hash `166f36ea…`, 12,227 raw + 583 joinable rows and EVERY other column byte-identical. The HEAD rehearsal reproduced today's real live hash exactly. ⚠ The ticket addendum's "exactly ONE spurious `vintage_changed`" is WRONG — the store holds 12 distinct hashes over the last 10 capture dates; a fresh hash is the norm. Pinned: old `season` spelling refused (one fixture moved), zero-eligible → honest `None`, one token carries no sub-cause. 17 new tests, red 15/17 first; suite 6795 / 33 skipped (6778 + 17). Going live needs a trunk pull only — no npm build, no API restart. | Fred |
...
| DG-139 | Served age is the model's feature-season age, not today's — 324 players a year young (255 scored); Wilson's row says 25 AND "2 years to the 28 cliff" | **2** | open — filed 09-02 15:32 by Tower (DG-130 scope side finding, re-measured); DG-137's rule applied to `age` at `universe_pvo_batch.py:198` + `roster_auditor.py:240`; plain `or` is right here (Sleeper None = unknown) | unclaimed |
