...
| DG-128 | Rank everyone: the 8-game gate refuses 115 players we already have history for | 3 | todo — **David ruled 09-01: coverage fix, no slot; pre-committed taper, not tuned; the band ships with the number.** ⚠ the earlier `-8047` stamp on this ticket was Tower's own inspection worktree (dg-work.sh 12:32), not a build claim — and until `79a3756`+1 every Lane PID on this board was dg-work.sh's OWN `$$`, dead at script exit, so no Lane PID before today is checkable. 114 of the 115 sit at `games_t` 4–7, inside the existing blend window; the fix is supplying `_dvs_a`, not widening the window. Depends on DG-127's columns being in data (09-02) | Davids-MacBook-Pro-77417 (claude pid, alive) |
| DG-129 | The availability model is not a published artifact, so a CSV edit changes served values | 3 | todo — David 09-01: not now | — |
| DG-130 | A player reads 100% complete and still has no score, and nothing says why | **6** (+3) | todo | — |
