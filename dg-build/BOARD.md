...
| DG-121 | Nothing detects a served bundle that has drifted from HEAD | **6** | todo — DG-076's deferred half, finally ticketed. This is the detector missing behind the week-stale bundle David hit 08-29; the remedy then was a README ritual, and a ritual is not a detector. Four honest states (current/drifted/dirty/unknown); a missing manifest is `unknown`, never `current` | — |
...
| DG-141 | `vintage_changed` is true every morning by construction — a microsecond timestamp is hashed into `provenance_hash`, poisoning the PAIR across days; `semantic_output_hash` still works and must not be touched | **2** | open — filed 09-02 22:3x. Measured by Fred, reproduced independently by Tower: 9 of 9 consecutive date pairs show a new vintage; `source_snapshot_captured_at` is hashed at `model_forward_capture_driver.py:158` while the docstring at `:138-142` says dates are excluded. Kills the "one-time flip" wording in DG-137 + DG-139. The 09-02 same-day pair proves `semantic_output_hash` alone works. | unclaimed |
...
| DG-143 | Lower the season-games gate 8 -> 4 so a player who missed most of a season still gets a number (114 unlocked) | **2** | **BUILT + PUSHED `ef08020c` on `ticket/DG-143`, NOT reviewed, NOT landed.** Market check is WEAKER than first reported (audit: 0.634-0.711 across seven snapshots vs a 0.788-0.805 baseline, n=32; the 0.711 was date-lucky) though 94% of the 82 the market ignores still score <20. ⛔ DO NOT LAND until David rules on the percentile shift — mean 6.8 pts on 329 of 468, but ALL UPWARD and NO reordering. Was — David's ruling 09-03 (option A of a written either/or) + "no 'partial season' lang": the number ships BARE. Wilson 68.5 / Allen 20.5 computed and shown to him. ⚠ percentiles are population-wide — measure the shift on his 27 and the top 50 BEFORE landing. The auditor's 3.28-vs-3.19 case is REFUTED and must not be cited. | Tower |
| DG-144 | Remove the range: one number per player (David 09-03 06:22 ET: "plus or minus 20, remove it") | **6** | doing — on-screen removal only; band fields stay on the API and PVO (post-freeze retirement is David's call) | Davids-MacBook-Pro-54105 |
